...
class REFER:
    def __init__(self, data_root, vis_root, dataset='refcoco', splitBy='unc'):
        ...
        logging.info('loading dataset %s into memory...' % dataset)
        ...
        # create index
        self.createIndex()
        logging.info('DONE (t=%.2fs)' % (time.time() - tic))
    ...
    def showRef(self, ref, seg_box='box'):
        ...
        for sid, sent in enumerate(ref['sentences']):
            logging.info('%s. %s' % (sid + 1, sent['sent']))
        # show segmentations
        if seg_box == 'seg':
            ann_id = ref['ann_id']
    ...
    def getMask(self, ref):
        ...
        # 	logging.info 'Finding closes mask point ...'
...
if __name__ == '__main__':
    ...
    logging.info(len(ref_ids))

    logging.info(len(refer.Imgs))
    logging.info(len(refer.imgToRefs))

    ref_ids = refer.getRefIds(split='train')
    logging.info('There are %s training referred objects.' % len(ref_ids))
    ...
    for ref_id in ref_ids:
        ...
        logging.info(ref)
        logging.info('The label is %s.' % refer.Cats[ref['category_id']])
        plt.figure()
