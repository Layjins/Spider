...
class T2VTsMulWebVid:
    ...
    def to_dict(self, sample):
        ...
        # Randomly sample 1 to 5 videos and corresponding texts
        num_videos_to_sample = random.randint(1, 5)  # Randomly choose 1 to 5 videos
        indices = random.sample(range(5), num_videos_to_sample)  # Randomly select indices for sampling

        # Define modalities for image, video, and audio
        modalities = {
        ...
        # Iterate over randomly sampled videos and their captions
        for i in indices:
            instruction = random.choice(self.instruction_pool)  # Each video gets a different instruction
            instruction_q = instruction

            # Initialize list to collect answer components for the current video
            current_answer_components = []
            ...
            if "<IMAGE>" in instruction:
                ...
                current_answer_components.append(modalities["<IMAGE>"].format(txt_batch[i]))
            if "<VIDEO>" in instruction:
                instruction_q = instruction_q.replace("<VIDEO>", "video")
                current_answer_components.append(modalities["<VIDEO>"].format(txt_batch[i]))
            if "<AUDIO>" in instruction:
                instruction_q = instruction_q.replace("<AUDIO>", "audio")
                current_answer_components.append(modalities["<AUDIO>"].format(txt_batch[i]))

            # If no modality tag is present, default to including all modalities
            if ("<IMAGE>" not in instruction) and ("<VIDEO>" not in instruction) and ("<AUDIO>" not in instruction):
                current_answer_components.append(modalities["<IMAGE>"].format(txt_batch[i]))
                current_answer_components.append(modalities["<VIDEO>"].format(txt_batch[i]))
                current_answer_components.append(modalities["<AUDIO>"].format(txt_batch[i]))

            # Construct the question and answer strings for the current video
            question_str += instruction_q.format(txt_batch[i]) + "."  # Append to the question string
            answer_str += "{}{}".format(txt_batch[i], "".join(current_answer_components)) + "."  # Append to the answer string
...
@registry.register_builder("t2v_ts_mul_webvid")
class T2VTsMulWebVidBuilder:
    train_dataset_cls = T2VTsMulWebVid

    def __init__(self, cfg=None):
        self.config = cfg

    def build_datasets(self):
        logging.info("Building T2VTsMulWebVid datasets...")
