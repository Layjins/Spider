...
#inner_dataset = wds.WebDataset(webdataset_path, resampled=True).decode(wds.torch_video).map(ProcessVideos()).to_tuple("video", "text").shuffle(1)
inner_dataset = wds.WebDataset(webdataset_path, resampled=True, handler=wds.warn_and_continue)\
    ...
    .shuffle(1, handler=wds.warn_and_continue) # 调式时，降低cpu内存占用
dataloader = DataLoader(inner_dataset, batch_size=1, num_workers=0)
