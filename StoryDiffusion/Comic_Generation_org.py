...
class SpatialAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        encoder_hidden_states=None, # prompt_embeds # (2,77,768) # 输入的prompt和negative prompt生成的embedding
        attention_mask=None,
        temb=None):
        # 全局变量，用于控制注意力机制的执行步骤
        ...
        # skip in early step
        if cur_step <5:
            # Consistent Self-Attention, attention_mask=None
            ...
            if random_number > rand_num:
                # 0.7或0.9的概率进行Consistent Self-Attention, 使用attention_mask
                if not write:
...
def story_generation(pipe, prompt_array=None):
    ### Set Config ###
    ## Global
...
if __name__ == "__main__":
    ...
    preds = story_generation(pipe, prompt_array)
